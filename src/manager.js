...
export class Manager {
  ...
  toggleStatus(elementId) {
    const element = this.getTodoById(elementId)
      ? this.getTodoById(elementId)
      : this.getProjectById(elementId);
  ...
  getOverdueTodos() {
    const overdueTodos = this.todos.filter(
      (todo) => isPast(todo.dueDate) && !todo.isComplete,
    );
  ...
  getCurrentTodos() {
    const notOverdueTodos = this.todos.filter(
      (todo) => !isPast(todo.dueDate) && !todo.isComplete,
    );
  ...
  getTodosByProject(projectId) {
    const todos = this.todos.filter(
      (todo) => todo.projectId === projectId && !todo.isComplete,
    );
    return todos;
  }
  ...
  getInboxTodos() {
    const todos = this.todos.filter(
      (todo) => !todo.projectId && !todo.isComplete,
    );
    return todos;
  }
  ...
  createProject(project = {}) {
    const projectExists = this.projects.find(
      (projectEl) => projectEl.id === project.id,
    );
    if (projectExists) {
  ...
  getProjectById(projectId) {
    const project = this.projects.find(
      (projectEl) => projectEl.id === projectId,
    );
    if (!project) {
  ...
  validateTodo(todo) {
    const errors = [];
    if (todo.projectId) {
      const isPresent = this.projects.some(
